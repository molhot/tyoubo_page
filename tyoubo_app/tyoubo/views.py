...
def create_post(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'tyoubo/post_form.html', {'item': form})
    if request.method == 'POST':
        form = PostForm(request.POST, instance=regist_item_obj)
        if form.is_valid():
            post = form.save(commit=False)
            post.save()
        return redirect('tyoubo:read_post')
...
def read_post(request):
    ...
    return render(request,'tyoubo/item_list.html',  {'items': tyoubo_items})
...
def edit_post(request, item_id):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'tyoubo/post_form.html', {'item': item, 'item_id': item_id})
...
class PostForm(ModelForm):
    ...
    class Meta:
        model = TyouboConfig
        fields = ('registed_item_name', 'item_price', 'item_type')
    #django.core.exceptions.FieldError: Unknown field(s) (item_name) specified for TyouboConfig
    #fieldsがあっていないと上記エラーが出る
    #まさかの名前は変数の方だった（ならitem_price = models.IntegerField('item_price')のitem_priceはなに？？）
